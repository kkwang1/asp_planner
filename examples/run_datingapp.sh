...
echo
clingo ../cpocl.lp datingapp.lp -t $CORES | tee datingapp_output.txt

# Reverse the file to get the latest output. The output is the next line after "Optimization: XXX"
OUTPUT=$(tac datingapp_output.txt | grep "Optimization:" -A 2 -m 1 | sed '2q;d')
echo
echo
echo "################ VISUALIZATION #################"
if [ -z "$OUTPUT" ]; then
    echo
    echo "Could not find an answer in datingapp_output.txt. Perhaps the run was canceled too soon?"
