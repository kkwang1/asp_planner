...
echo
clingo ../cpocl.lp uksocialmedia.lp -t $CORES | tee uksocialmedia_output.txt

# Reverse the file to get the latest output. The output is the next line after "Optimization: XXX"
OUTPUT=$(tac uksocialmedia_output.txt | grep "Optimization:" -A 2 -m 1 | sed '2q;d')
echo
echo
echo "################ VISUALIZATION #################"
if [ -z "$OUTPUT" ]; then
    echo
    echo "Could not find an answer in uksocialmedia_output.txt. Perhaps the run was canceled too soon?"
