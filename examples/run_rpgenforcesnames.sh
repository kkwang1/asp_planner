...
echo
clingo ../cpocl.lp rpgenforcesnames.lp -t $CORES | tee rpgenforcesnames_output.txt

# Reverse the file to get the latest output. The output is the next line after "Optimization: XXX"
OUTPUT=$(tac rpgenforcesnames_output.txt | grep "Optimization:" -A 2 -m 1 | sed '2q;d')
echo
echo
echo "################ VISUALIZATION #################"
if [ -z "$OUTPUT" ]; then
    echo
    echo "Could not find an answer in rpgenforcesnames_output.txt. Perhaps the run was canceled too soon?"
